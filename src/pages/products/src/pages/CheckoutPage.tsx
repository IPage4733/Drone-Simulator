  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const item of items) {
        ...
        // Add zone plan specific data if available
        if (item.zones && item.zones.length > 0) {
          payload.zones = item.zones;  // Already an array from PlanCard
          payload.numberOfPCs = item.numberOfPCs || 1;
          console.log('Zone plan checkout with:', payload);
        }
